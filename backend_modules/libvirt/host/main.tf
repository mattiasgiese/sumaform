...
resource "libvirt_volume" "data_disk" {
  ...
  size  = var.repository_disk_size * 1024 * 1024 * 1024
...
resource "libvirt_domain" "domain" {
  ...
  dynamic "disk" {
    for_each = concat(
      ...
      length(libvirt_volume.data_disk) == var.quantity && var.repository_disk_size > 0 ? [{"volume_id" : libvirt_volume.data_disk[count.index].id}] : []
    )
    content {
